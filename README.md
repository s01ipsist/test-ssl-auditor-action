...
![Example audit outcome](audit_example.png)

## Features

- 🔍 Audit testssl.sh JSON output files
- 📋 Configurable security rules
- ⚙️ Flexible rule configuration (JSON)
...
2. **Fail Builds on Violations**: Set `fail-on-violation: 'true'` to alert if deployments become non-compliant
...
5. **Review Regularly**: Schedule periodic reviews of your rules configuration to align with evolving security standards. Note that testssl.sh itself uses a [grade rating system](https://github.com/ssllabs/research/wiki/SSL-Server-Rating-Guide) which is independently maintained and can act as a proxy for evolving SSL standards.
